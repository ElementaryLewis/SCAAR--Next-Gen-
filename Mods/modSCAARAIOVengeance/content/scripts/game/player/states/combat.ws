...
state Combat in CR4Player extends ExtendedMovable 
{
	...
	entry function PerformEvade( playerEvadeType : EPlayerEvadeType, isRolling : bool )
	{
		...
		if ( evadeTarget )
		{
			...
			else
			{
				...
				else //SCAAR Start
				{
					...
					if ( !theInput.IsActionPressed('SCAARDodge') && theInput.LastUsedPCInput() && playerToRawAngleDiff < 90.f || ( theInput.IsActionPressed('SCAARDodge') && theInput.LastUsedPCInput() || theInput.LastUsedGamepad() ) && playerToRawAngleDiff < 40.f ) //30.f
					{
					...
					else if ( ( theInput.IsActionPressed('SCAARDodge') && theInput.LastUsedPCInput() || theInput.LastUsedGamepad() ) && playerToRawAngleDiff < 120.f ) //135.f
					{
						if ( dodgeLeftOrRight < 0.f )
						{
							dodgeDirection = PED_Right;
							//theGame.GetGuiManager().ShowNotification( dodgeLeftOrRight );
						}
		...
		//if ( !parent.GetWeaponHolster().IsMeleeWeaponReady() ) //SCAAR
		//{ //SCAAR
			//dodgeNum = 0;	//SCAAR
		//} //SCAAR
		if ( !turnInPlaceBeforeDodge ) //SCAAR
		{
	...
	entry function ProcessAttack( playerAttackType : name, performApproachAttack : bool )
	{
			...
			&& ( VecDistance( parent.GetWorldPosition(), npc.GetNearestPointInBothPersonalSpaces( parent.GetWorldPosition() ) ) < 1.5f || !isDeadlySwordHeld ) )
		{
			if ( !isDeadlySwordHeld )
			{
				...
				//if ( parent.RaiseEvent( 'CombatTaunt' ) ) //SCAAR
