...
state Combat in CR4Player extends ExtendedMovable 
{
	...
	entry function CombatInit()
	{
		...
		parent.AddTimer( 'SCAARCombatLoop', 1.f, true );
	...
	timer function SCAARCombatLoop( timeDelta : float , id : int )
	{
		//var pressTime : float;
		SCAARApplySettingsOnStateSwitch(); //SCAAR
		if ( theInput.IsActionPressed('SCAARTaunt') )
		{
				SCAARCallTaunt();						//SCAAR
				//pressTime = theGame.GetEngineTimeAsSeconds();
		}
		//if ( !theInput.IsActionPressed('SCAARTaunt') )
		//{
			//if ( theGame.GetEngineTimeAsSeconds() - pressTime < 0.5f )
			//{
				//SCAARApplySettingsOnStateSwitch();		//SCAAR
			//}
		//}
	...
	entry function PerformEvade( playerEvadeType : EPlayerEvadeType, isRolling : bool )
	{
		...
		var dodgeLeftOrRight			: float; //SCAAR
		...
		if ( evadeTarget )
		{
			...
			dodgeLeftOrRight = AngleNormalize180( AngleDistance( rawDodgeHeading, parent.GetHeading() ) ); // SCAAR
			...
			else
			{
				...
				else //SCAAR Start
				{
					...
					if ( ( !theInput.IsActionPressed('SCAARDodge') && theInput.LastUsedPCInput() || !thePlayer.IsGuarded() && theInput.LastUsedGamepad() ) && playerToRawAngleDiff < 90.f || ( theInput.IsActionPressed('SCAARDodge') && theInput.LastUsedPCInput() || thePlayer.IsGuarded() && theInput.LastUsedGamepad() ) && playerToRawAngleDiff < 40.f ) //30.f
					{
					...
					else if ( ( theInput.IsActionPressed('SCAARDodge') && theInput.LastUsedPCInput() || thePlayer.IsGuarded() && theInput.LastUsedGamepad() ) && playerToRawAngleDiff < 120.f ) //135.f
					{
						if ( dodgeLeftOrRight < 0.f )
						{
							dodgeDirection = PED_Right;
						}
						else if ( dodgeLeftOrRight > 0.f )
						{
							dodgeDirection = PED_Left;
						}
					} //SCAAR End
					else
					{
	...
	entry function ProcessAttack( playerAttackType : name, performApproachAttack : bool )
	{
			...
			&& ( VecDistance( parent.GetWorldPosition(), npc.GetNearestPointInBothPersonalSpaces( parent.GetWorldPosition() ) ) < 1.5f || !isDeadlySwordHeld ) )
		{
			if ( !isDeadlySwordHeld )
			{
				...
				if ( parent.RaiseEvent( 'CombatTaunt' ) && !SCAAR_GetVoiceTauntsEnabled() ) //SCAAR
					parent.PlayBattleCry( 'BattleCryTaunt', 1.f, true, true ); //SCAAR
